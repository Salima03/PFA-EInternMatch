...
const NvprofileStudent = () => {
  ...
  return (
    ...
    <div style={{ backgroundColor: "#fffff", minHeight: "100vh", padding: "30px 0" }}>
      <div style={{
...
const NvprofileStudent = () => {
  ...
  const fetchProfile = async () => {
    try {
      ...
      setProfile(data);
      setEmail(data.email || ''); // Récupération directe de l'email depuis l'API
      await Promise.all([
        fetchProfilePicture(),
        fetchCoverPhoto(),
        fetchCv(),
  ...
  return (
    <div className="layout"> 
      <Sidebar />
      <main className="page-content">
        <div style={{ backgroundColor: "##e0f2f1", minHeight: "100vh", padding: "30px 0" }}>
          <div style={{
            backgroundColor: "#ffffff",
            width: "90%",
            maxWidth: "1000px",
            ...
            <div style={{ display: "flex", flexWrap: "wrap", gap: "20px", margin: "0 40px 20px" }}>
              {/*<ReadOnlyField label="Email" value={email} />*/}
              {profile.phone && <ReadOnlyField label="Téléphone" value={profile.phone} />}
              {profile.location && <ReadOnlyField label="Localisation" value={profile.location} />}
            </div>

            {/* About Section */}
            {profile.summary && (
              <div style={{ padding: "20px 40px" }}>
                <h3 style={{ borderBottom: "1px solid #eee", paddingBottom: "10px" }}>About me</h3>
                <p style={{ marginTop: "10px", color: "#555", lineHeight: "1.6" }}>{profile.summary}</p>
              </div>
            )}
